...
import { Phone, Video, MessageSquare, CalendarPlus } from "lucide-react";
...
const MotivatorCard = ({ motivator }: { motivator: { name: string, specialty: string, avatar: string, status: string }}) => (
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-semibold font-headline">Brothers</h2>
        <div className="grid grid-cols-1 gap-4">
          {brothers.map((motivator) => (
            <MotivatorCard key={motivator.name} motivator={motivator} />
          ))}
        </div>
      </div>

       <div className="space-y-4">
        <h2 className="text-lg font-semibold font-headline">Sisters</h2>
        <div className="grid grid-cols-1 gap-4">
          {sisters.map((motivator) => (
            <MotivatorCard key={motivator.name} motivator={motivator} />
          ))}
        </div>
      </div>
